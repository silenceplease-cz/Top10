...
Zranitelnost typu injektování je chyba aplikace, která umožňuje odeslat nedůvěryhodný uživatelský vstup do interpretu (např. prohlížeči, databázi nebo příkazové řádce) a způsobuje, že interpret provede části tohoto vstupu jako příkazy. 

Aplikace je zranitelná vůči útoku, pokud:

* Uživatelem dodaná data nejsou aplikací validována, filtrována ani sanitizována.
* Dynamické dotazy nebo volání bez parametrizace bez kontextově závislého escapování jsou používány přímo v interpretu.
* Neočištěná (nesanitizovaná) data jsou použita v parametrech vyhledávání ORM (objektově-relační mapování) k vytažení dalších citlivých záznamů.
...
Nejlepším způsobem, jak zabránit injekci, je oddělit data od příkazů a dotazů:
...
**Scénář #2:** Slepá důvěra aplikace ve frameworky může vést k dotazům, které jsou stále zranitelné. Například Hibernate Query Language (HQL):
